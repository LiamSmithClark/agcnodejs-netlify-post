      ...
      if(hrefAttr==null){
        ...
        if(hostnameHref==hostnameComing){
          ...
          if(isUrl(hrefAttr)==false){
            ...
            const pathHref = mapHref.pathname+mapHref.query;
            const dataReplace = option.origin+"/post-"+protoHref+hostnameHref+pathHref;
            $(el).attr(option.remake,dataReplace);
          };
        };
      };
...
app.use( async (req,res,next)=>{
  ...
  try{
    if("/ping"==req.url){
      res.status(200);
      res.send("ok");
    }else if(req.url.indexOf(dataSetting["name-folder-sitemap"])>0&&req.url.indexOf(".xml")>0&&req.url.indexOf("/post-")==-1&&req.method === "GET"){
...
<!-- XML Sitemap generated by NodeJs -->`);
      ...
      res.end(data);
    }else if (req.url.split("/assets/")[1] == undefined == false && req.method === "GET" && req.url.indexOf("/post-")==-1) {
      ...
      try{
        ...
        let dataOrigin="";
        if(urlPost.indexOf("post-")==0){
          let dataLink=await urlPost.split("post-")[1];
          linkPost=await dataLink.replace("https-","https://").replace("http-","http://");
          statusOrigin=false;
          dataOrigin=await "post-"+parseUrl(linkPost).origin.replace("https://","https-").replace("http://","http-");
        };
        if(isUrl(linkPost)){
